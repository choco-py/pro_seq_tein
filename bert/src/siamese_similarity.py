...
# Model Classes --------------------------------------------------------------
class SiameseSim(NeuralNetworkModel):
    ...
    def input(
        ...
        with tf.name_scope('input'):
            ...
            next_batch = ((X_fp1_batch, X_fp2_batch,  X_fp3_batch,  X_fp4_batch,
                X_fp5_batch, X_fp6_batch, X_fp7_batch,  X_label_batch),
                (Y_fp1_batch, Y_fp2_batch, Y_fp3_batch,  Y_fp4_batch,
                Y_fp5_batch, Y_fp6_batch, Y_fp7_batch,  Y_label_batch)) = data_op.get_next()


        print('[dtype] fingerprint1: %s , fingerprint2: %s, fingerprint3: %s, fingerprint4: %s, fingerprint5: %s, fingerprint6: %s , fingerprint7: %s' % (X_fp1_batch.dtype, X_fp2_batch.dtype, X_fp3_batch.dtype, X_fp4_batch.dtype, X_fp5_batch.dtype, X_fp6_batch.dtype, X_fp7_batch.dtype))
        print('[shape] fingerprint1: %s , fingerprint2: %s, fingerprint3: %s, fingerprint4: %s, fingerprint5: %s, fingerprint6: %s , fingerprint7: %s' % (Y_fp1_batch.get_shape(), Y_fp2_batch.get_shape(),X_fp3_batch.get_shape(), X_fp4_batch.get_shape(), X_fp5_batch.get_shape(), Y_fp6_batch.get_shape(), Y_fp7_batch.get_shape()))
    ...
    def build_SiamNet(
        ...
        with tf.variable_scope('SiameseNet_model', reuse=reuse):
            ...
            Input_similarity = tf.placeholder(
                tf.float32,
                shape=[488, 488],
                name='seq_similarity',
            )
            ...
            with tf.name_scope('spring_loss_scope'):
                Reshape_X_label = tf.reshape(
                    Input_X_label,
                    [-1, 1],
                )
                Reshape_Y_label = tf.reshape(
                    Input_Y_label,
                    [-1, 1],
                )
                Input_XY_label = tf.concat(
                    [Reshape_X_label,
                    Reshape_Y_label],
                    axis=1,
                    name='label_concat',
                ...
                similarity_value =tf.gather_nd(
                    params=Input_similarity,
                    indices=[Input_XY_label],
                    name='similarity_value',
                    # batch_dims=self.BATCH_SIZE,
                )
                similarity_score = tf.add(
                    similarity_value,
                    1,
                )
                protein_eucl = tf.multiply(
                    eucl_distance_0,
                    similarity_score,
                    name='protein_eucl',
                )
                    ...
                    try:
                        for batch in range(train_len):
                            ((X_fp1_batch, X_fp2_batch,  X_fp3_batch,  X_fp4_batch,
                                X_fp5_batch, X_fp6_batch,X_fp7_batch,  X_label_batch),
                                (Y_fp1_batch, Y_fp2_batch, Y_fp3_batch,  Y_fp4_batch,
                                Y_fp5_batch, Y_fp6_batch, Y_fp7_batch, Y_label_batch)) = sess.run(self.next_batch)
                            ...
                            batch_value = " similarity_value: %.5f" % (np.round(np.mean(similarity_value),2))
                            batch_score = " similarity_score: %.5f" % (np.round(np.mean(similarity_score),2))
                            # batch_dist = f'distance: {np.round(distance, 2)}'
                            batch_msg = batch_msg + batch_bar + batch_err + batch_value +batch_score
